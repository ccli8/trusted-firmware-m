...
uint32_t SDH_Read(SDH_T *sdh, uint8_t *pu8BufAddr, uint32_t u32StartSec, uint32_t u32SecCount)
{
    ...
    while(u32Loop > 0UL)
    {
        ...
        /* Initialize timeout */
        SDH_CLR_INT_FLAG(sdh, SDH_INTSTS_RTOIF_Msk | SDH_INTSTS_DITOIF_Msk);
        sdh->TOUT = 0xFFFFFF;
#endif
...
#else
            if(SDH_GET_INT_FLAG(sdh, SDH_INTSTS_BLKDIF_Msk))
#endif
            {
                break;
            }
...
#if NU_SDH_ENA_TIMEOUT
            /* Check timeout */
            if (SDH_GET_INT_FLAG(sdh, SDH_INTSTS_RTOIF_Msk | SDH_INTSTS_DITOIF_Msk))
            {
                SDH_CLR_INT_FLAG(sdh, SDH_INTSTS_RTOIF_Msk | SDH_INTSTS_DITOIF_Msk);
                return SDH_ERR_DEVICE;
            }
